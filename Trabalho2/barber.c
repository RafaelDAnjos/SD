...
void* thread_cliente(void* argc){
    ...
    if(*(cliente->v)==0 && barbeiro_desocupado(cliente)==1 && sem_trywait(cliente->acorda_barber)!=0){
        /*printf("A fila esta vazia, o cliente %d vai acordar algum barbeiro\n",cliente->id);*/
        /*A fila esta vazia e existe algum barbeiro desocupado, então o cliente da um post no semaforo acorda_barber,
        enviando um sinal para algum barbeiro acordar*/
...
void* thread_barbeiro(void* argc){
    ...
    while(continua_rodando(barber->num_barbeiros)){
        ...
        *(barber->visor) = barber->id;
        ...
        /*caso tenha clientes esperando nas cadeiras de clientes o barbeiro informa aos clientes que ele desocupou*/
        if(*(barber->v)>0){
